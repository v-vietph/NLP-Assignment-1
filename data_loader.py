...
vocab_dict = {key: val.values for key, val in frame.T.items()}
...
def word2tensor(word_list):
    embeddings = []
    for word in word_list:
        try:
            embeddings.append(vocab_dict[word].astype(np.float32))
        except KeyError:
            embeddings.append(unknown_vector.astype(np.float32))
...
class WordEmbeddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        line = line.split()
        embedding_tensors = word2tensor(line[1:-2])
        # print("line", line)
        if len(line) == 0:

...
def train(train_data_dir = './data/train.txt', model_to_train = 'RNN', n_epochs=20, learning_rate=0.001):

    word_dataset = WordEmbeddingDataset(train_data_dir)
    dataloader = DataLoader(word_dataset, batch_size=1,
                            shuffle=True, num_workers=4)
    print("Training model", model_to_train)
    if model_to_train == 'RNN':
        train_RNN(dataloader=dataloader, device=device,n_epochs = n_epochs, learning_rate = learning_rate)
    elif model_to_train == 'LSTM':
        train_LSTM(dataloader=dataloader, device=device,n_epochs = n_epochs, learning_rate = learning_rate)
    elif model_to_train == 'GRU':
        train_GRU(dataloader=dataloader, device=device,n_epochs = n_epochs, learning_rate = learning_rate)
...
if __name__ == '__main__':
    ...
    train(args.train_data, args.model_to_train, args.n_epochs, args.learning_rate)
